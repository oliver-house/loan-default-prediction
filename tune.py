...
from src.config import RANDOM_STATE, TARGET_COL, ID_COL
...
def main() -> None:
    ...
    # Print as a copy-pasteable dict
    print("\n── Best params (copy into src/config.py) ───")
    print("LGBM_PARAMS = {")
    ...
    print('    "n_jobs":        -1,')
    for k, v in best.params.items():
        fmt = f'"{v}"' if isinstance(v, str) else v
        print(f'    "{k}": {fmt},')
